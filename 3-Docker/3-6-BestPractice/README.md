...
特に新しいレイヤーを追加する命令については、レイヤーキャッシュのコントロールに関わるので注意して使います。
...
ビルド時に必要な秘匿情報の受け渡しにはシークレットマウントを検討してください。

また、各Dockerイメージビルドサービスでは秘匿情報を受け渡すオプションが提供されている場合が多いです。
...
## 特権コンテナを避ける
Dockerのrunコマンドにprivilegedオプションを付与することで起動することで、特権コンテナとしてコンテナが起動します。
...
## rootでの起動を避ける
コンテナはホストマシンのカーネルを共有しています。

## Dockerイメージのバージョンを注意深く管理する


# 参考資料
zembutsuさんのまとめがわかりやすいです。
